...
def set_min(array, val1 = 0.05, val2=1.0):
    for j in  range(array.shape[0]):
      for i,x in enumerate(array[j]):
        if x<val1:
            array[j,i] = val1
        elif x>val2:
            array[j,i] = val2
...
#    tokenizer = AutoTokenizer.from_pretrained("state-spaces/mamba-1.4b-hf")
    ...
    for model in tqdm(models, desc="Model", leave=False, disable=args.hide_progress):
      ...
      for x in range(100):
        ...
        beta = 0.9
        import math
        alpha = 0.005 * math.cos(x * math.pi/(99 * 2))
        delta = numpy.random.choice([-1,1], size=(48,8))
...
#                vectors_large.append(t)
            result.append(ppl)

        result.insert(0, model)
        results.append(result)

#    numpy.save('vectors_large.npy', numpy.array(vectors_large))
    if args.output_file:
